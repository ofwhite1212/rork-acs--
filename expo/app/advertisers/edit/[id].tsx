...
export default function EditAdvertiserScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  const { advertisers, updateAdvertiser } = useAdvertiserStore();
  ...
  useEffect(() => {
    if (advertiser) {
      ...
      setAdContent(advertiser.adContent);
      setPreferredArea(advertiser.preferredArea);
      setPeriod(advertiser.period);
      setBudget(advertiser.budget.toString());
      setContactPerson(advertiser.contactPerson);
      setContactInfo(advertiser.contactInfo);
    }
  }, [advertiser]);
  ...
  const handleSubmit = () => {
    if (validate()) {
      ...
      if (Platform.OS === 'web') {
        if (window.confirm('広告主情報を更新しました')) {
          router.replace(`/advertisers/${id}`);
        } else {
          router.replace(`/advertisers/${id}`);
        }
      } else {
        Alert.alert("更新完了", "広告主情報を更新しました", [
          {
            text: "OK",
            onPress: () => {
              router.replace(`/advertisers/${id}`);
